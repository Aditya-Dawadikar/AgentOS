...
def _start_container_monitor(container_id, log_path, kill_switch_path, lifecycle_log_path, event_since):
    ...
    if os.name == 'nt':
        ...
        if Path(pythonw_executable).exists():
            monitor_executable = pythonw_executable

    monitor_command = [monitor_executable,
                       str(project_dir / 'container_monitor.py'),
                       container_id,
                       str(log_path),
...
def run_job(job_id):
    ...
    container_data_dir = data_root / container_name
    ...
    _start_container_monitor(container.id, log_path, kill_switch_path, lifecycle_log_path, event_since)
    container.start()
